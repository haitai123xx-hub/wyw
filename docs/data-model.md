...
- `status = 'valid'` 时必须满足上述坐标和原文一致性；`needs-review` 表示原文修改与标注范围相交，旧文字和批注内容仍会保留，但不会在正文中高亮。
...
| 标注粒度 | 可用批注类型 |
| --- | --- |
| 字 | 释义、一词多义、古今异义、词类活用、通假字、文言虚词 |
| 词 | 释义、一词多义、古今异义、词类活用 |
| 句 | 释义、特殊句式 |

这套规则同时用于前端选项、后端新增/修改校验和分享包导入校验。同一区间可以保存多条批注，同一种类型也可以保存多条，彼此不会覆盖。

### 修改原文与批注迁移

保存修改后的原文时，后端计算旧文本和新文本的共同前缀、共同后缀及中间修改区：

- 修改区之前的批注保持原坐标。
- 修改区之后的批注按字符差值自动平移。
- 与修改区相交的批注改为 `needs-review`，保留原选中文字、类型和内容。
- 用户可在批注目录打开待定位批注，在新原文中重新选择位置并保存；成功后恢复为 `valid`。

一次包含多处分散改动的编辑会把两端共同文本之间整体视为修改区，这种保守策略可能让更多批注进入待定位，但不会静默删除笔记。

原文和批注仍是关联数据，应通过应用提供的原文编辑入口触发迁移；不建议绕过应用直接修改 JSON，否则无法计算坐标变化。
