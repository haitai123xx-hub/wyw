...
- 创建多个项目，并用带颜色的分组整理项目。
- 选中文本后，按“字 / 词 / 句”记录批注对象；不同粒度只显示适用的批注类型。
- 七类批注：释义、一词多义、古今异义、词类活用、通假字、文言虚词、特殊句式。
- 为每类批注分别设置文字颜色、背景颜色、字体、字号、粗体、下划线和斜体；这些设置是本机用户预设，会应用于所有文章。
- 编辑、删除批注，以及导入、导出单个项目的分享文件。
- 同一段原文可连续添加多条不同批注；修改原文时自动迁移未受影响的批注，冲突批注会保留并等待重新定位。
- 左侧篇目区域和右侧批注区域均可隐藏、恢复和拖拽调宽，布局会自动记忆。
...
- `projects/<uuid>.json` 保存某篇文章的完整正文和批注；显示时统一使用 `library.json` 中属于当前用户的批注样式预设。
- 分享文件只有在用户主动导出时才写入用户选择的位置；只有在用户主动导入时才读取。
- 分享文件不包含字体、颜色和标记方式，导入后按接收者自己的本机预设显示。
